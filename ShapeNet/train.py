...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader),
        ...
        batch_shapeious = compute_overall_iou(seg_pred, target,
                                              num_part)  # list of current batch_iou:[iou1,iou2,...,iou#b_size]
    ...
    if args.scheduler == 'cos':
        scheduler.step()
        # scheduler = optim.lr_scheduler.CosineAnnealingLR(opt, T_max=20, eta_min=0.05)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='DSNS')
    ...
    parser.add_argument('--test_batch_size', type=int, default=32, metavar='batch_size',
                        help='Size of batch)')
    parser.add_argument('--epochs', type=int, default=351, metavar='N',
