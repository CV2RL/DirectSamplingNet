...
class PointMLP(nn.Module):
    def __init__(self, num_classes=50,points=2048, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        ### Building Dicrect Upsample #####
        self.upsample = nn.ModuleList()
        up_dims = [576, 320, 192]  # de_dims[i]+64
        for i in range(len(en_dims)-2):
            self.upsample.append(
                PointNetFeaturePropagation(up_dims[i], de_dims[3],
                                           blocks=de_blocks[i], groups=groups, res_expansion=res_expansion,
                                           bias=bias, activation=activation)
            )
        ...
        self.classifier = nn.Sequential(
            nn.Conv1d(gmp_dim+cls_dim+de_dims[-1]*4, 128, 1, bias=bias),
            nn.BatchNorm1d(128),
            nn.Dropout(),
    ...
    def forward(self, x, norm_plt, cls_label):
        ...
        for i in range(len(self.decode_list)):
            ...
            if i < 3:
                # print('i:{}, after_decoder_x.shape:{}'.format(i, x.shape))
                upsample_x = self.upsample[i](xyz_list[3+1], xyz_list[i+1], x_list[3+1], x)
                x_Upsample.append(upsample_x)
                # print('i:{}, x.shape:{}'.format(i, x.shape))

        multi_feature = torch.cat(x_Upsample, dim=1)
        ...
        x = torch.cat([x, global_context.repeat([1, 1, x.shape[-1]]), multi_feature, cls_token.repeat([1, 1, x.shape[-1]])], dim=1)
